...
pub fn handle_message(store: &dyn DocbrainStore, db_path: &Path, raw: &str) -> Option<String> {
...
fn handle_tools_call(store: &dyn DocbrainStore, db_path: &Path, id: Value, params: &Value) -> JsonRpcResponse {
...
mod tests {
    ...
    #[test]
    fn tools_list_matches_the_single_source_of_truth_table() {
        let store = SqliteDocbrainStore::open_in_memory().unwrap();
        let resp = handle_message(&store, Path::new("unused.db"), r#"{"jsonrpc":"2.0","id":2,"method":"tools/list","params":{}}"#).unwrap();
        let v: Value = serde_json::from_str(&resp).unwrap();
        assert_eq!(v["result"]["tools"].as_array().unwrap().len(), tools::list_tools().len());
    }
