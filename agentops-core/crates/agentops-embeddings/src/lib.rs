//! Local embedding generation via `fastembed` — no external service. Shared
//! by `docbrain-ingest` (doc-chunk embeddings) and codebrain's semantic
//! search (`agentops-graph`'s `search_similar`) — extracted here rather
//! than each maintaining its own copy, and the single source of truth for
//! `EMBEDDING_DIM` both `docbrain-graph`'s `sqlite-vec` table and
//! `agentops-graph`'s `node_vectors`/`PostgresGraphStore`'s `vector(384)`
//! column size against.
//!
//! Model is loaded once per process and reused; the Hugging Face download
//! on first use is the intentional, documented exception in `deny.toml`'s
//! `wrappers`-scoped `reqwest` allowance.
//!
//! `Embedder` is the port; `LocalEmbedder` is the one adapter today. Use
//! cases depend on `&dyn Embedder`, never on `fastembed` or `LocalEmbedder`
//! directly — a remote/API-backed embedder could be added later as a
//! second adapter without touching them.
...
/// BGE-small-en-v1.5 — 384 dimensions. Every embedding this crate produces,
/// and every vector-column/table sized against it, must agree on this.
...
/// BGE-small-en-v1.5 — chosen for a light local footprint over a larger
/// model like BGE-M3, since this runs inline in the ingestion/query path
/// with no dedicated GPU assumed.
fn model() -> Result<&'static Mutex<TextEmbedding>> {
    ...
    // parallel threads, in docbrain-ingest before this code moved here).
    // `INIT_LOCK` serializes the init section itself so only one thread
    // ever downloads/loads the model.
...
/// Embeds one piece of text, matching `EMBEDDING_DIM`. Not `pub` — the port
/// (`Embedder`) and its adapter (`LocalEmbedder`) are the public surface;
/// this is `LocalEmbedder`'s implementation detail.
...
mod tests {
    ...
    // the one thing other crates' tests deliberately fake out, so it needs
    // its own live check that the real path actually works end to end.
    #[test]
