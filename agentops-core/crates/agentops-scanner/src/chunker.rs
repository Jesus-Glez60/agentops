...
/// Produces chunks for a file: always a file-header chunk (first 30 lines), plus
/// either one chunk per symbol (truncated at 150 lines) if any symbols were
/// found, or a sliding-window fallback (150-line window, 20-line overlap) if
/// none were — matching the plan's chunker.rs behavior.
...
mod tests {
    ...
    fn emits_one_chunk_per_symbol_plus_header() {
        let source = "def a():\n    pass\n\ndef b():\n    pass\n";
        let symbols = vec![
            Symbol { name: "a".into(), kind: "function".into(), start_line: 1, end_line: 2, source: "def a():\n    pass".into() },
            Symbol { name: "b".into(), kind: "function".into(), start_line: 4, end_line: 5, source: "def b():\n    pass".into() },
        ];
        let chunks = chunk_file(source, &symbols);
        assert_eq!(chunks.len(), 3); // header + 2 symbols
    ...
    fn truncates_long_symbols_at_150_lines() {
        ...
        assert_eq!(chunk.text.lines().count(), MAX_SYMBOL_LINES + 1); // + truncation marker line
    }
    ...
    fn falls_back_to_sliding_window_when_no_symbols_found() {
        ...
        // header + at least 2 window chunks for a 400-line file with 130-line steps
        assert!(chunks.len() >= 3);
        assert!(chunks[1..].iter().all(|c| c.kind == ChunkKind::Window));

        // Windows should overlap: second window's start should be before first window's end.
