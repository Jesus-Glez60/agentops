...
impl CredentialStore {
    ...
    pub fn store_credential(&self, secrets: &dyn SecretsProvider, tenant: &str, new: NewCredential<'_>) -> Result<()> {
        let encrypted_secret = encrypt(secrets, tenant, new.secret)?;
        let encrypted_refresh_token = new.refresh_token.map(|t| encrypt(secrets, tenant, t)).transpose()?;
    ...
    pub fn get_credential(&self, secrets: &dyn SecretsProvider, tenant: &str, provider: &str) -> Result<Option<DecryptedCredential>> {
        ...
        let secret = decrypt(secrets, tenant, &encrypted_secret)?;
        let refresh_token = encrypted_refresh_token.map(|blob| decrypt(secrets, tenant, &blob)).transpose()?;
...
fn decrypt(secrets: &dyn SecretsProvider, tenant: &str, blob: &[u8]) -> Result<Zeroizing<String>> {
    ...
    let plaintext = cipher.decrypt(nonce, ciphertext).map_err(|e| anyhow::anyhow!("decrypting credential (wrong tenant key, or tampered/corrupted data): {e}"))?;
...
mod tests {
    ...
    fn decrypting_with_the_wrong_tenant_key_fails_rather_than_silently_returning_garbage() {
        ...
        let ciphertext = encrypt(&secrets, "tenant-a", "secret").unwrap();
        assert!(decrypt(&secrets, "tenant-b", &ciphertext).is_err());
    }
}
