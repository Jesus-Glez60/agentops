...
//!
//! Ported from `main`'s `agentops-embeddings` (pre-dating this rebuild's
//! open-core `agentops-embeddings` — a different, sqlite-vec/pgvector-backed
//! crate — hence this crate's rename to `agentops-heavy-embeddings`).
//! Adapted in three places against the current codebase, not ported
//! verbatim: `GraphStore::nodes_by_kind` is repo-scoped now (`main`'s
//! wasn't); `NodeKind::as_str()` is `as_db_str()` today; and
//! `docbrain_graph::DocbrainStore` is a trait with no `TenantContext`
//! parameter (single-tenant this rebuild — see `docbrain-graph`'s own doc
//! comment), not the tenant-scoped concrete type `main` had.
//!
...
/// The repo identity a scanned graph's nodes are actually stored under —
/// canonicalized directory name, matching `agentops_mcp::scan::repo_name`
/// exactly (that function lives in the root workspace, not reachable from
/// here). **Callers must use this, not a raw filesystem path**, for both
/// indexing (`collect_index_items`'s `repo` argument) and searching
/// (`SemanticIndex::search`'s `repo` filter) — a real bug caught via live
/// testing against this repo itself: `nodes_by_kind` is repo-scoped now
/// (unlike `main`, where this mismatch was harmless because nothing
/// filtered by it), so passing the raw path instead of this silently
/// indexed/found zero nodes, no error anywhere.
pub fn repo_name(path: &std::path::Path) -> String {
    path.canonicalize().ok().and_then(|p| p.file_name().map(|n| n.to_string_lossy().into_owned())).unwrap_or_else(|| path.display().to_string())
}

/// Reads every Symbol/Gotcha/Decision node out of `store` (repo-scoped —
/// `GraphStore::nodes_by_kind` requires it today, unlike `main`'s unscoped
/// original) into embeddable `IndexItem`s — File nodes are skipped, since
/// they have no meaningful text of their own (their symbols already carry
/// that). Deliberately a plain synchronous function, not a method on
/// `SemanticIndex`: it takes `&dyn GraphStore`, which isn't provably `Sync`
/// (`SqliteGraphStore` wraps a `rusqlite::Connection`, intentionally `!Sync`
/// upstream), so a reference to it must never be held across an `.await` —
/// keeping this synchronous and separate from `SemanticIndex::index_items`
/// (the actual async embedding step) is what makes that impossible to get
/// wrong by accident.
///
/// `repo` must be the value from [`repo_name`], not a raw filesystem path —
...
pub fn collect_index_items(store: &dyn agentops_graph::GraphStore, repo: &str) -> Result<Vec<IndexItem>> {
    ...
    for kind in [NodeKind::Symbol, NodeKind::Gotcha, NodeKind::Decision] {
        for node in store.nodes_by_kind(repo, kind).context("reading nodes to index")? {
...
///
/// No tenant parameter, unlike `main`'s original — `docbrain-graph` is
/// single-tenant this rebuild (see its own module doc comment); multi-tenant
/// isolation for hosted docbrain search is achieved at the deployment layer
/// instead (one database per tenant, routed by connection string — see the
/// 1.0 roadmap plan's Phase 1 decision), not inside this trait.
///
...
pub fn collect_doc_index_items(store: &dyn docbrain_graph::DocbrainStore, slug: &str) -> Result<Vec<IndexItem>> {
    let nodes = store.all_doc_nodes(slug).context("reading doc nodes to index")?;
    Ok(nodes
        .into_iter()
        .map(|node| IndexItem {
...
mod tests {
    ...
    // Hits a real, in-memory SqliteDocbrainStore (no network) — verifies
    ...
    // Adapted from main's version: no TenantContext (single-tenant this
    // rebuild), add_library takes 4 args, and nodes are added via
    // upsert_doc_node (the only insertion path the current trait exposes).
    #[test]
    fn collect_doc_index_items_reads_real_doc_nodes_and_namespaces_ids() {
        ...
        store
            .upsert_doc_node(
                "next",
                NewDocNode { version: "15.1.3", topic: "routing", content: "App router basics", content_hash: "abc123", token_count: 4, embedding: None, kind: DocNodeKind::Prose },
            )
            .unwrap();

        let items = collect_doc_index_items(&store, "next").unwrap();
